...
if os.name in ('nt', 'dos'):  # If Machine is running on Windows, use mscvrt
    import msvcrt
else:
    import getch

def get_input():     # A function that gets the input of a char and auto presses enter.
    if os.name in ('nt', 'dos'):  # If Machine is running on Windows, use mscvrt
        inp = msvcrt.getch()
        return inp
    else:
...
inp =  get_input()   # Get the only one letter input and the user doesn't need to press enter.
